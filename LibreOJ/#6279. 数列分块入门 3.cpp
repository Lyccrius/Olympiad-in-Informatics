...
class BlockArray {
	private:
		...
		int AskArray(int l, int r, int val) {
			int ret = - 1;
			for (int i = block[array[l].bel].l; i <= block[array[r].bel].r; i++) {
				if (l <= array[i].pos && array[i].pos <= r) {
					if (array[i].val + block[array[i].bel].tag < val) {
						ret = std::max(ret, array[i].val + block[array[i].bel].tag);
					}
				}
			}
			return ret;
		...
		int AskBlock(int l, int r, int val) {
			int ret = - 1;
			for (int i = array[l].bel + 1; i <= array[r].bel - 1; i++) {
				int pos = std::lower_bound(array + block[i].l, array + block[i].r + 1, (Array) {val - block[i].tag}, cmpByVal) - array;
				if (pos > block[i].l) {
					ret = std::max(ret, array[pos - 1].val + block[i].tag);
				}
			}
			return ret;
		}
	...
	public:
		...
		int ask(int l, int r, int val) {
			int ret = - 1;
			if (array[l].bel == array[r].bel) {
				return AskArray(l, r, val);
			}
			ret = std::max(ret, AskArray(l, block[array[l].bel].r, val));
			ret = std::max(ret, AskArray(block[array[r].bel].l, r, val));
			ret = std::max(ret, AskBlock(l, r, val));
			return ret;
		}
} BA;
