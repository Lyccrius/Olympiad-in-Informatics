...
int cnt[maxN + 10][maxM + 10][maxN + maxM + 10];
...
void calc(int v, int u, int k) {
    ...
    for (int j = k; j <= n + m - v - u; j++) {
        for (int i = std::max(0, u + j - m); i <= std::min(j, n - v); i++) {
            ...
            // n - v >= u + j - m
            // j <= n + m - v - u
            a[v + i][u - i + j] = (a[v + i][u - i + j] + 1ll * c[j][i] * cnt[v][u][k] % mod) % mod;
        }
    }
    return;
}
...
int main() {
    ...
    for (int v = 1; v <= n; v++) {
        for (int u = 1; u <= m; u++) {
            for (int k = 0; k < n + m - v - u + 1; k++) {
                if (cnt[v][u][k]) calc(v, u, k);
            }
        }
    }
    for (int i = 1; i <= n; i++) {
