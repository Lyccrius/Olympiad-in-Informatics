...
namespace graph {
	...
	void addEdge(int tail, int head, str text) {
		edges.push_back((Edge) {false, tail, head, text});
	...
	void DFS(int u, int fe) {
		...
		for (auto e : vertex[u].edge) {
			int v = edges[e].head;
            if (edges[e].ban) continue;
            edges[e].ban = true;
			vertex[u].edge.pop_front();
			DFS(v, e);
		}
		if (~fe) ans.push_back((Node) {u, fe});
        //if (~fe) printf("ans.push "), std::cout << edges[fe].text, printf("\n");
        //printf("DFS %c ", u + 'a' - 1); if (~fe) std::cout << edges[fe].text; printf(" end \n");
		return;
	}

	void print() {
        if (!ans.empty()) std::cout << edges[ans.back().e].text, ans.pop_back();
		while (!ans.empty()) {
            std::cout << '.';
			std::cout << edges[ans.back().e].text;
			ans.pop_back();
		}
		return;
	}
