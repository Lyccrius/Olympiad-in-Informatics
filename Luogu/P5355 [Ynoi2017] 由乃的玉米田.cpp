...
struct CaptaimMo {
    ...
    void sqrAdd(int pos, int x) {
        if (cnt[a[pos]] == 0) {
            may[a[pos]] = true;
            if (a[pos] * x <= maxA) yam[a[pos] * x] = true;
        }
        cnt[a[pos]]++;
        return;
    }

    void sqrDel(int pos, int x) {
        cnt[a[pos]]--;
        if (cnt[a[pos]] == 0) {
            may[a[pos]] = false;
            if (a[pos] * x <= maxA) yam[a[pos] * x] = false;
        }
        return;
    }
    ...
    void sqrSol(int x) {
        ...
        int r = 0;
        for (int i = 1; i <= m; i++) {
            if (query[i].opt != 4 || query[i].x != x) continue;
            while (l > query[i].l) l--, sqrAdd(l, x);
            while (r < query[i].r) r++, sqrAdd(r, x);
            while (l < query[i].l) sqrDel(l, x), l++;
            while (r > query[i].r) sqrDel(r, x), r--;
            ans[query[i].id] = (may & yam).any();
            query[i].x = 0;
        }
        may.reset();
        yam.reset();
        std::fill(cnt, cnt + maxA + 10, 0);
        return;
    }
