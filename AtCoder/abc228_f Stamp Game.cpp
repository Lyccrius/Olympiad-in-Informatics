...
int main() {
    ...
    for (int i = 1; i + h2 - 1 <= h; i++) for (int k = w1 - w2 + 1; k > 1; k -= k / 2) for (int j = 1; j + w2 - 1 + k / 2 <= w; j++) d[i][j] = std::max(d[i][j], d[i][j + k / 2]);
    for (int j = 1; j + w2 - 1 <= w; j++) for (int k = h1 - h2 + 1; k > 1; k -= k / 2) for (int i = 1; i + h2 - 1 + k / 2 <= h; i++) d[i][j] = std::max(d[i][j], d[i + k / 2][j]);
